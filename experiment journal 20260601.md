...
## 元数据同步状态修正（2026-06-01 15:58 +08:00）
...
- 已按 SHA-256 内容哈希识别精确重复文件，并只在源目录内部删除重复副本：
  - 第一次运行在写 manifest 前中断，但已先删除 4 个精确重复文件。
  - 修复脚本后继续删除 332 个精确重复文件，释放约 0.63GB。
  - 源目录整体从 2574 个文件减少到 2238 个文件，减少 336 个；清理后约 14.136GB。
